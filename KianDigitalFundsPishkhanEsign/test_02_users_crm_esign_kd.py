...
def test_get_oppourtunity_by_application_id():
    get_opportunity_id_response = helper_functions.get_oppourtunity_by_application_id(BASE_URL, CRM_TOKEN,ONBOARDING_TOKEN_CREDENTIAL, APPLICATION_NAME)
...
def test_do_action_rq():
    ...
    get_do_action_rq_response = helper_functions.do_action_rq(BASE_URL, CRM_TOKEN, ONBOARDING_TOKEN_CREDENTIAL, APPLICATION_NAME)
...
def test_get_opportunity_identification():
    get_opportunity_identification_response = helper_functions.get_opportunity_identification(BASE_URL, CRM_TOKEN, ONBOARDING_TOKEN_CREDENTIAL, APPLICATION_NAME)
...
def test_submit_check_identification():
    submit_check_identification_response = helper_functions.submit_check_identification(BASE_URL,CRM_TOKEN,  ONBOARDING_TOKEN_CREDENTIAL, APPLICATION_NAME)
...
#     get_additional_info_response = helper_functions.get_additional_info(BASE_URL, CRM_TOKEN, ONBOARDING_TOKEN_CREDENTIAL, APPLICATION_NAME)
...
def test_confirm_by_qc():
    confirm_by_qc_response = helper_functions.confirm_by_qc(BASE_URL,CRM_TOKEN,ONBOARDING_TOKEN_CREDENTIAL,APPLICATION_NAME)
    # print(confirm_by_qc_response.json())
    assert confirm_by_qc_response.status_code == 200


def test_finish_pipeline():
    finish_pipeline_response = helper_functions.finish_pipeline(BASE_URL, CRM_TOKEN, ONBOARDING_TOKEN_CREDENTIAL, APPLICATION_NAME)
